...
|Command                               |T_code|Parameters |Descriptiom                    |
|--------------------------------------|------|-----------|-------------------------------|
|CMD_SPEED_CTRL                        |      | T, L, R
|CMD_PWM_INPUT                         |	  | L, R 
|CMD_ROS_CTRL                          |	  | X, Z
|CMD_SET_MOTOR_PID                     |	  | P, I, D, L
|CMD_OLED_CTRL                         |	3 |	          | Control text on OLED display  |
|                                      |      | lineNum   | Line to place text 0..3       |
|                                      |      | Text      | Text to display               |
|CMD_OLED_DEFAULT                      |   -3 | Restore OLED Screen                       |
|                                      |      | (no parameters)                           |
|CMD_MODULE_TYPE                       |	  | cmd
|CMD_GET_IMU_DATA                      |      | -
|CMD_CALI_IMU_STEP                     |      | -
|CMD_GET_IMU_OFFSET                    |	  | x, y, z
|CMD_BASE_FEEDBACK                     |      | -
|CMD_BASE_FEEDBACK_FLOW                |	  | cmd
|CMD_FEEDBACK_FLOW_INTERVAL            |	  | cmd
|CMD_UART_ECHO_MODE                    |	  | cmd
|CMD_ARM_CTRL_UI                       |	  | E, Z, R
|CMD_LED_CTRL                          |	  | IO4, IO5
|CMD_GIMBAL_CTRL_SIMPLE                |	  | X, Y, SPD, ACC
|CMD_GIMBAL_CTRL_MOVE                  |	  | X, Y, SX, SY
|CMD_GIMBAL_CTRL_STOP                  |	  | -						
|CMD_HEART_BEAT_SET                    |	  | cmd
|CMD_GIMBAL_STEADY                     |	  | s, y
|CMD_SET_SPD_RATE                      |	  | L, R
|CMD_GET_SPD_RATE                      |      | -
|CMD_SAVE_SPD_RATE                     |      | -
|CMD_GIMBAL_USER_CTRL                  |	  | X, Y, SPD

	// EoAT type settings.
|CMD_EOAT_TYPE                         |	  | mode	
|CMD_CONFIG_EOAT                       |	  | pos, ea, eb

	// it moves to goal position directly
	// with interpolation.
|CMD_MOVE_INIT                         |	  | joint, rad, spd, acc
|CMD_JOINTS_RAD_CTRL                   |	  | base, shoulder, elbow, hand, spd, acc
|CMD_SINGLE_AXIS_CTRL                  |	  | axis, pos, spd
|CMD_XYZT_GOAL_CTRL                    |	  | x, y, z, t, spd
|CMD_XYZT_DIRECT_CTRL                  |	  | x, y, z, t
|CMD_SERVO_RAD_FEEDBACK                |      | -
|CMD_EOAT_HAND_CTRL                    |	  | cmd, spd, acc
|CMD_EOAT_GRAB_TORQUE                  |	  | tor							
|CMD_SET_JOINT_PID                     |	  | joint, p, i
	
	// set a new x-axis.
	
|CMD_SET_NEW_X                         |	  |	xAxisAngle
|CMD_DELAY_MILLIS                      |	  |	cmd
|CMD_DYNAMIC_ADAPTATION                |	  | mode, b, s, e, h	
|CMD_SINGLE_JOINT_ANGLE                |	  | joint, angle, spd, acc
|CMD_CONSTANT_CTRL                     |	  | m, axis, cmd, spd

	// mission & steps edit & file edit.
|CMD_SCAN_FILES                        |
|CMD_CREATE_FILE                       |	  | name, content
|CMD_READ_FILE                         |	  | name
|CMD_DELETE_FILE                       |	  | name
|CMD_APPEND_LINE                       |	  | name, content
|CMD_INSERT_LINE                       |	  | name, lineNum, content
|CMD_REPLACE_LINE                      |	  | name, lineNum, content
|CMD_READ_LINE                         |	  | name, lineNum
|CMD_DELETE_LINE                       |	  | name, lineNum
|CMD_TORQUE_CTRL                       |	  | cmd

|CMD_CREATE_MISSION                    |	  | name, intro
|CMD_MISSION_CONTENT                   |	  | name
|CMD_APPEND_STEP_JSON                  |	  | name, step
|CMD_APPEND_STEP_FB                    |	  | name, spd
|CMD_APPEND_DELAY                      |	  | name, delay
|CMD_INSERT_STEP_JSON                  |	  | name, stepNum, step
|CMD_INSERT_STEP_FB                    |	  | name, stepNum, spd
|CMD_INSERT_DELAY                      |	  | name, stepNum, delay
|CMD_REPLACE_STEP_JSON                 |	  | name, stepNum, step
|CMD_REPLACE_STEP_FB                   |	  | name, stepNum, spd
|CMD_REPLACE_DELAY                     |	  | name, stepNum, delay
|CMD_DELETE_STEP                       |	  | name, stepNum
|CMD_MOVE_TO_STEP                      |	  | name, stepNum
|CMD_MISSION_PLAY                      |	  | name, times
	
	// esp-now settings.
|CMD_BROADCAST_FOLLOWER                |	  | mode, mac
|CMD_ESP_NOW_CONFIG                    |	  | mode
|CMD_ESP_NOW_ADD_FOLLOWER              |	  | mac
|CMD_ESP_NOW_REMOVE_FOLLOWER           |      | mac
|CMD_ESP_NOW_GROUP_CTRL                |	  | dev, b, s, e, h, cmd, megs
|CMD_ESP_NOW_SINGLE                    |	  | mac, dev, b, s, e, h, cmd, megs
  	
	// wifi settings.
|CMD_WIFI_ON_BOOT                      |	  | cmd
|CMD_SET_AP                            |	  | ssid, password
|CMD_SET_STA                           |	  | ssid, password
|CMD_WIFI_APSTA                        |	  | ap_ssid, ap_password, sta_ssid, sta_password
|CMD_WIFI_INFO                         |	  | mode, ap_ssid, ap_password, sta_ssid, sta_password
|CMD_WIFI_CONFIG_CREATE_BY_STATUS      |      | -
|CMD_WIFI_STOP 	                       |      | -

	// servo settings.
|CMD_SET_SERVO_ID                      |	  | raw, new,
|CMD_SET_MIDDLE                        | 	  | id
|CMD_SET_SERVO_PID                     |	  | id, p, 
